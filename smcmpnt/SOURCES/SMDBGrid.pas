...
{ Copyright (C) 1998-2022, written by Mike Shkolnik, Scalabium Software
...
{used only if BLOB fields with graphics contains the jpeg's}
//{$DEFINE USE_JPEG}

{$IFDEF USE_DBEXPRESS}
	{$IFNDEF VER_ENTERPRISE}
...
uses
  ...
  Menus, Grids, DBGrids, {$IFDEF USE_TNT} TntDBGrids, {$ENDIF} DB, StdCtrls, SMCnst,
  {$IFDEF SMForDelphi6} Variants, {$ENDIF}
  {$IFDEF SMForDelphiXE2} System.Types, System.UITypes, {$ENDIF}
  {$IFDEF USE_JPEG} Jpeg, {$ENDIF}
  RXUtils {ex-VCLUtils from RX-Lib};

type
  TSMGString = {$IFDEF USE_TNT} WideString {$ELSE} string{$ENDIF};
  ...
  TExOptions = set of (eoAutoWidth, eoBooleanAsCheckBox,
                       ...
                       eoShowFilterBar, eoAnyKeyFilter, eoFilterAutoApply, eoTitleLines,
                       eoCellWordWrap);
  ...
  // Event to intercept the filtering process. I.e. for columns with different content drawn in an grid's OnDraw...-event.
  TOnFilterApply = procedure (Sender: TObject; Field: TField; const FilterString: string; var Accept: boolean) of object; {MS1}
...
type
  ...
  TSMGVerticalAlignment = (gvaTop, gvaCenter, gvaBottom);

  TSMDBColumn = class({$IFDEF USE_TNT} TTntColumn {$ELSE} TColumn{$ENDIF})
  private
    FBandIndex: Integer;

    FSortCaption: TSMGString;
    FSortType: TSMSortType;
    FInplaceEditor: TSMInplaceEditorType;

    FFooterValue: Variant;
    FFooterType: TSMFooterType;
    FFooterText: TSMGString;

    FTag: Longint;
    FTextEllipsis: TSMTextEllipsis;

    FFilterValue: TSMGString;
    FFilterList: TStrings;

    // Mark a column to be able to get filtered. Eliminates the filter box for this column.
    FFilterable: Boolean;

    FVerticalAlignment: TSMGVerticalAlignment;

    procedure SetFooterValue(Value: Variant);
    procedure SetFooterType(Value: TSMFooterType);
    procedure SetSortCaption(Value: TSMGString);
    procedure SetSortType(Value: TSMSortType);
    procedure SetInplaceEditor(Value: TSMInplaceEditorType);
    procedure SetFilterValue(const Value: TSMGString);
    procedure SetFooterText(const Value: TSMGString);
  ...
  published
    ...
    property SortCaption: TSMGString read FSortCaption write SetSortCaption;
    ...
    property FooterText: TSMGString read FFooterText write SetFooterText;

    property Tag: Longint read FTag write FTag default 0;
    property TextEllipsis: TSMTextEllipsis read FTextEllipsis write FTextEllipsis default teNone;

    property FilterValue: TSMGString read FFilterValue write SetFilterValue;
    property FilterList: TStrings read GetFilterList write SetFilterList;
    property Filterable: Boolean read FFilterable write FFilterable default True;

    property VerticalAlignment: TSMGVerticalAlignment read FVerticalAlignment write FVerticalAlignment default gvaTop;
  end;
  ...
  private
    FValueChecked: TSMGString;
    FValueUnChecked: TSMGString;
  protected
    procedure SetValueChecked(const AValue: TSMGString);
    procedure SetValueUnChecked(const AValue: TSMGString);
  public
    constructor Create(Collection: TCollection); override;
    procedure DrawItem(ACanvas: TCanvas; ARect: TRect; Value: Variant; Background: TColor; AState: TGridDrawState; var Done: Boolean); override;
  published
    property ValueChecked: TSMGString read FValueChecked write FValueChecked;
    property ValueUnChecked: TSMGString read FValueUnChecked write FValueUnChecked;
  end;

  TSMDBGridColumns = class({$IFDEF USE_TNT} TTntDBGridColumns {$ELSE} TDBGridColumns{$ENDIF})
  private
  ...
  published
    ...
    property Selection: TSMGradientDraw index 5 read GetGradient write SetGradient;
  ...
  published
    ...
    property Expression: TSMGString read FExpression write FExpression;
  ...
  TDrawFooterCellEvent = procedure(Sender: TObject; Canvas: TCanvas;
                                   ...
                                   var FooterText: TSMGString; var DefaultDrawing: Boolean) of object;

  TGetParsedExpression = procedure(Sender: TObject; Expression: TSMGString; var Text: TSMGString; var Value: Boolean) of object;
  TDrawGroupingCellEvent = procedure(Sender: TObject; ACanvas: TCanvas; CellRect: TRect;
                                     Group: TSMGrouping; Text: TSMGString; var DefaultDrawing: Boolean) of object;
  ...
  TSMDBGridFilterComboBox = class(TComboBox)
  public
    Column: TSMDBColumn;
  end;
  ...
  {$ENDIF}
  TSMDBGrid = class({$IFDEF USE_TNT} TTntDBGrid {$ELSE} TDBGrid{$ENDIF})
  private
    { Private declarations }
    FBands: TStrings;
    FBandsFont: TFont;
    ...
    FOnFilterApply: TOnFilterApply;

    FHintField: TSMGString;
    ...
    FSelectionAnchor: {$IFDEF SMForDelphi2009}TBytes{$ELSE}TBookmarkStr{$ENDIF};
    ...
    CmbFilter: TSMDBGridFilterComboBox;
    ...
    {original idea found in TBitDBGrid:
...
//    procedure WMPaint(var Message: TWMPaint); message WM_PAINT;
    ...
    procedure CmbFilterKeyPress(Sender: TObject; var Key: Char);
...
{$ENDIF}
    ...
    function GetCheckBoxValue(AField: TField; Value: Variant; ValueChecked: TSMGString): TCheckBoxState;
    ...
    function GetVerticalAlignment(AColumn: {$IFDEF USE_TNT} TTntColumn {$ELSE} TColumn{$ENDIF}): TSMGVerticalAlignment;
...
{$ENDIF}
  public
    ...
    procedure CalculateTotals(OnlySelected: Boolean {$IFDEF SMForDelphi5}=False{$ENDIF});
    ...
    function SetFilterFocus(AColumn: TColumn): Boolean;

    procedure ClearSort;
    procedure SetSortField(AField: TField; ASortType: TSMSortType);

    procedure FocusFilterBar;
    function FilterIsEmpty: Boolean;
    procedure ClearFilter;
    procedure ApplyFilter;

    procedure ResyncIndicator; {MS1}
    procedure CheckFilter; {MS1}
    procedure UpdateLayout; {MS1}
  ...
  published
    ...
    property HintField: TSMGString read FHintField write FHintField;
    property ShowMemoText: Boolean read FShowMemoText write FShowMemoText default False;
    ...
    property OnChangeSelection: TNotifyEvent read FOnSelectionChange write SetOnChangeSelection; //{$IFDEF SMForDelphi6} deprecated {$IFDEF SMForDelphi2009} 'Use OnSelectionChange instead' {$ENDIF} ; {$ENDIF}
...
procedure SaveGridToIni(grid: TCustomDBGrid; const FileName: string; const SectionName: string {$IFDEF SMForDelphi5} = '' {$ENDIF} );
procedure LoadGridFromIni(grid: TCustomDBGrid; const FileName: string; const SectionName: string {$IFDEF SMForDelphi5} = '' {$ENDIF} );
...
implementation
uses {$IFDEF SMForDelphiXE2} System.Win.Registry, {$ELSE} Registry, {$ENDIF}
     IniFiles,
     ExtCtrls, Clipbrd, ExtDlgs, TypInfo
     {$IFDEF USE_BDE}, DBTables {$ENDIF}
     {$IFDEF USE_DBEXPRESS}, DBLocalS {$ENDIF}
     {$IFDEF VER_ENTERPRISE}, DBClient {$ENDIF}
     {$IFDEF VER_KBMMEMTABLE}, kbmMemTable {$ENDIF}
     {$IFDEF USE_TNT} , TntWindows, TntGraphics {$ENDIF}
     {$IFDEF SMForDelphiXE2} , Vcl.Themes {$ELSE} {$IFDEF SMForDelphi2010} , Themes {$ENDIF}{$ENDIF}
     ;
...
{$ENDIF};
      ...
      {$ENDIF}
        if Column.Field.Dataset is {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF} then
        begin
          with {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF}
            (Column.Field.Dataset) do
          begin
    ...
    {$IFDEF VER_ENTERPRISE}
      ...
      if Column.Field.Dataset is {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF} then
      begin
        with {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF} (Column.Field.Dataset) do
        begin
          if Assigned(MasterSource) and (MasterFields <> EmptyStr) then
          begin
...
{$ENDIF}
    {$IFDEF USE_BDE}
      ...
      begin
        with TTable(Column.Field.Dataset) do
          if Exclusive then
            AddIndex(SMIndexName, AllFields, IdxOpts{$IFDEF SMForDelphi4}, DescFields {$ENDIF})
          else
            exit
      end
    {$ENDIF}
    ...
    {$ENDIF}
      if Column.Field.Dataset is {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF} then
        {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF}
            (Column.Field.Dataset).AddIndex(SMIndexName, AllFields, IdxOpts{$IFDEF SMForDelphi4}, DescFields {$ENDIF})
    {$ENDIF}
    {$IFDEF USE_KBMMEMTABLE}
      else
      if Column.Field.Dataset is TkbmMemTable then
...
begin
  ...
  {$ENDIF}
  {$IFDEF VER_ENTERPRISE}
     or (Column.Field.Dataset is {$IFDEF USE_DBEXPRESS} TSQLClientDataset {$ELSE} TClientDataset {$ENDIF})
  {$ENDIF}
...
begin
  ...
  try
    RegIniFile.WriteInteger(RegistrySection, 'Count', THackDBGrid(grid).Columns.Count);
    ...
    begin
      ...
      if (grid is TSMDBGrid) and (THackDBGrid(grid).Columns.Items[i] is TSMDBColumn) then
        with TSMDBColumn(THackDBGrid(grid).Columns.Items[i]) do
        begin
          ...
          if FFooterText<>'' then
            RegIniFile.WriteString(RegistrySection, IntToStr(i) + '_Footer',Format('%s',[NormalizedText(FFooterText)]));
        end;
    end;
  finally
    RegIniFile.Free;
...
procedure WriteTitleText(ACanvas: TCanvas; ARect: TRect; DX, DY: Integer;
  const Text: TSMGString; Alignment: TAlignment; VAlignment: TSMGVerticalAlignment; TextEllipsis: TSMTextEllipsis; IsWordBreak: Boolean {$IFDEF SMForDelphi4} ; ARightToLeft: Boolean {$ENDIF});
const
  ...
  VAlignFlags: array [TSMGVerticalAlignment] of Integer =
     (0{DT_VTOP},
      DT_VCENTER,
      0{DT_VBOTTOM});
{$IFDEF SMForDelphi4}
...
{$ENDIF}
    begin
      ...
      Flags := EllipsisFlags[TextEllipsis] or
               ...
               {$IFDEF SMForDelphi4} or RTL[ARightToLeft] {$ENDIF};
      if (VAlignment = gvaBottom) then
      begin
        Flags := Flags or DT_CALCRECT;

        {$IFDEF USE_TNT}
        intTextHeight := Tnt_DrawTextW(ACanvas.Handle, PWideChar(Text), Length(Text), ARect, Flags);
        {$ELSE}
      ...
      {$IFDEF USE_TNT}
      Tnt_DrawTextW(ACanvas.Handle, PWideChar(Text), Length(Text), ARect, Flags);
      {$ELSE}
...
{$ENDIF}
      ...
      Flags := EllipsisFlags[TextEllipsis] or AlignFlags[Alignment] {$IFDEF SMForDelphi4} or RTL[ARightToLeft] {$ENDIF} ;
      if (VAlignment = gvaBottom) then
      begin
        Flags := Flags or DT_CALCRECT;

        {$IFDEF USE_TNT}
        intTextHeight := Tnt_DrawTextW(Handle, PWideChar(Text), Length(Text), R, Flags);
        {$ELSE}
      ...
      {$IFDEF USE_TNT}
        Tnt_DrawTextW(Handle, PWideChar(Text), Length(Text), R, Flags);
      {$ELSE}
...
begin
  ...
  begin
    ...
    try
      ...
      Filterable := colSource.Filterable;
    finally
      if Assigned(Collection) then
...
procedure TSMDBColumn.SetSortCaption(Value: TSMGString);
begin
...
procedure TSMDBColumn.SetFooterText(const Value: TSMGString);
begin
...
procedure TSMDBColumn.SetFilterValue(const Value: TSMGString);
begin
...
begin
  ...
  begin
    strValue := {$IFDEF SMForDelphi6}VarToWideStr{$ELSE}VarToStr{$ENDIF}(Value);

    s := '';
    for i := 1 to Length(strValue) do
      s := s + PasswordChar;
    WriteTitleText(ACanvas, ARect, 0, 0, s{Value}, Alignment, VerticalAlignment,
                   TextEllipsis,
                   (eoCellWordWrap in grid.ExOptions) //False
                   {$IFDEF SMForDelphi4},
...
begin
  ...
  begin
    ...
    WriteTitleText(ACanvas, ARect, 0, 0, strValue, Alignment, VerticalAlignment,
                   TextEllipsis,
                   (eoCellWordWrap in grid.ExOptions) //False
                   {$IFDEF SMForDelphi4},
                   (grid.BiDiMode <> bdLeftToRight)
...
begin
  ...
  if IsXPThemesEnabled and
     {$IFDEF SMForDelphi2010} (Grid.FInternalDrawingStyle = gdsThemed) and {$ENDIF}
...
procedure TSMDBCheckboxColumn.SetValueChecked(const AValue: TSMGString);
begin
...
procedure TSMDBCheckboxColumn.SetValueUnChecked(const AValue: TSMGString);
begin
...
begin
  ...
  FSelection := TSMGradientDraw.Create(AGrid);
...
function TSMDBGridStyle.IsDefaultBackground: Boolean;
begin
...
//  Color := clInfoBk;
  ...
  {Filter editor}
  CmbFilter := TSMDBGridFilterComboBox.Create(Self);
  with CmbFilter do
  begin
    Parent := Self;
...
procedure TSMDBGrid.SetOnChangeSelection(Value: TNotifyEvent);
begin
  FOnSelectionChange := Value;

  { WARNING: You are using the deprecated OnChangeSelection event - use OnSelectionChange instead. }
end;
...
begin
  Result := TSMDBGridColumns.Create(Self, {$IFDEF SMForDelphi5} TSMDBColumn {$ELSE} TColumn {$ENDIF});
...
//  GetCellProps(SelectedIndex, DataLink.ActiveRecord, Columns[SelectedIndex].Field, nil, Background, False);
...
begin
  if (eoShowFilterBar in ExOptions) and
     (CmbFilter.Visible or not TSMDBColumn(Column).Filterable) then
    exit;
...
procedure TSMDBGrid.CalculateTotals(OnlySelected: Boolean {$IFDEF SMForDelphi5}=False{$ENDIF});
var
  ABookmark: TBookmark;
  i: Integer;
...
function TSMDBGrid.GetCalculatedDefaultRowHeight: Integer;
var
  K: Integer;
...
begin
  ...
  begin
    if not (dgRowLines in Options) and (eoTitleLines in ExOptions) then
        ...
        begin
          ...
          MaxHeight := Max(MaxHeight,
                           DrawText(Canvas.Handle,
                                    ...
                                    Length(s),
                                    RRect,
                                    DT_EXPANDTABS or DT_CALCRECT or WordBreaks[eoCellWordWrap{eoTitleWordWrap} in ExOptions]));
        end;
      end;
...
procedure TSMDBGrid.ApplyStyleColors(ACanvas: TCanvas);
begin
  {$IFDEF SMForDelphiXE2}
  if TStyleManager.IsCustomStyleActive then
  begin
    if True {$IFDEF SMForDelphiXE3} and (seClient in StyleElements) {$ENDIF} then
      ACanvas.Brush.Color := StyleServices.GetStyleColor(scGrid)
    else
      ACanvas.Brush.Color := Color;
    if True {$IFDEF SMForDelphiXE3} and (seFont in StyleElements) {$ENDIF} then
...
procedure TSMDBGrid.DrawFooterRow;
var
  DrawBitmap: TBitmap;
  ...
  begin
    ...
    begin
      if (Columns[BCol] is TSMDBColumn) then
        ...
        begin
          if (Field is TNumericField) and (TNumericField(Field).DisplayFormat <> '') and
              {$IFDEF SMForDelphi6}
              VarIsNumeric(FooterValue)
              {$ELSE}
              (VarType(FooterValue) in [varSmallInt, varInteger, varSingle, varDouble, varCurrency, varByte])
              {$ENDIF}
                         then
            strFooter := FormatFloat(TNumericField(Field).DisplayFormat, FooterValue)
          else
          if (Field is TDateTimeField) and (TDateTimeField(Field).DisplayFormat <> '') and
              (VarType(FooterValue) in [varDate]) then
            DateTimeToString(strFooter, TDateTimeField(Field).DisplayFormat, FooterValue)
          else
            strFooter := {$IFDEF SMForDelphi6}VarToWideStr{$ELSE}VarToStr{$ENDIF}(FooterValue);
          if FFooterText <> '' then
          begin
    {$IFDEF SMForDelphi4}
...
{      FooterCellRect.Top := FooterCellRect.Top +
      ...
      WriteTitleText(ACanvas, FooterCellRect, 0, 0, strFooter, al, gvaCenter,
                     teNone,
                     False
                     {$IFDEF SMForDelphi4},
...
begin
  ...
  if FAutoFitIsLocked or not (eoShowFooter in ExOptions) then exit;
...
{$IFDEF SMForDelphiXE2}
  if TStyleManager.IsCustomStyleActive {$IFDEF SMForDelphiXE3} and (seClient in StyleElements) {$ENDIF} then
    ACanvas.Brush.Color := StyleServices.GetStyleColor(scGrid);
{$ENDIF}
  ACanvas.FillRect(FooterRect);
...
procedure TSMDBGrid.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
  inherited;
...
{procedure TSMDBGrid.WMPaint(var Message: TWMPaint);
var
  DC, MemDC: HDC;
    ...
    begin
      ...
      try
        ...
        try
          ...
          if Message.DC = 0 then InvalidateFooter;  // fix for black footer??
        finally
...
{  if ScrollBars in [ssNone, ssHorizontal] then
  ...
  {$IFDEF SMForDelphi2010} if (FInternalDrawingStyle = gdsThemed) then {$ENDIF}
  begin
...
{20050714}
  if (Groupings.Count > 0) or
     ...
     not GridStyle.IsDefaultBackground then
...
{copy from TGXDBGrid component, gnunn@GExperts.com}
...
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        HintTxt := SRecNo +
          FormatFloat('#,###', DataLink.DataSet.RecNo) + //IntToStr(DataLink.DataSet.RecNo) +
          SRecOf +
          FormatFloat('#,###', GetRecordCount); //IntToStr(GetRecordCount);
        GetCursorPos(pt);
...
begin
  ...
  begin
    ...
    case Field.DataType of
      ...
      ftMemo,
      ftFmtMemo
      {$IFDEF SMForDelphi2006}, ftWideMemo{$ENDIF}: Result := Integer(gpMemo);
...
procedure TSMDBGrid.SMSelectionChanging;
begin
  if Assigned(FOnSelectionChanging) then
    FOnSelectionChanging(Self);
end;

procedure TSMDBGrid.SMSelectionChanged;
begin
...
procedure TSMDBGrid.CMFontChanged(var Message: TMessage);
begin
  inherited;
  LayoutChanged;
end;
   ...
   with PHintInfo(Msg.LParam)^  do
     try
       ...
       begin
         ...
         // start - use the event to get the text for the hint
  ...
  begin
    ...
    try
      ...
      for i := 0 to ColCount-1 do
        {$IFDEF SMForDelphi4}
        if Columns[i].Visible then // ignore hidden columns
        {$ENDIF}
...
procedure TSMDBGrid.UpdateLayout;
begin
  BeginLayout;
  LayoutChanged;
  EndLayout;
...
var
  ...
  ArrayOfColumnsLen: array {$IFDEF SMForDelphi5} {$ELSE} [0..200] {$ENDIF} of Integer;
  StartPoint: Integer;
  ...
  function GetBiggestWord(Title: TSMGString): TSMGString;
  var
    BiggestCount: Integer;
    Count: Integer;
    BiggestWord: TSMGString;
  begin
    BiggestCount := 0;
    while Title <> '' do
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if ColumnIsCheckbox(Columns[i]) or not Assigned(Columns[i].Field) then
              TempLen := 2
            else
...
{$ENDIF}
  ...
  begin
    ...
    begin
      if (Value is TComponent) {$IFDEF SMForDelphi6}and
        not (csSubComponent in TComponent(Value).ComponentStyle) {$ENDIF} then
      begin
...
begin
  ...
  if (PropInfo^.GetProc <> nil) and
     ...
     ((PropInfo^.PropType^.Kind = tkClass) and
      (TObject(GetOrdProp(Instance, PropInfo)) is TComponent) {$IFDEF SMForDelphi6}and
...
procedure TSMDBGrid.ResyncIndicator;
var
  r: Integer; {MS1} // Repaint/Sync after some Locate/GotoBookmark-OPs when indicator is not on active record
begin
  try
    Repaint;
...
procedure TSMDBGrid.GetCellProps(ACol, ARow: Integer; Field: TField; AFont: TFont;
...
begin
  ...
  begin
    ...
    if (eoFixedLikeColumn in ExOptions) or
      ...
      begin
        if (ARow{DataLink.ActiveRecord} mod 2 = 0) then
...
begin
  ...
  if (Groupings.Count > 0) or
     (GridStyle.Style <> gsNormal) or
     not GridStyle.IsDefaultBackground then
//     (Assigned(GridStyle.Background) and not GridStyle.Background.Empty) or {MS1} // avoid problem with empty graphic
//     (GridStyle.Wallpaper.Direction <> fdNone) then
  begin
    Invalidate
  ...
  begin
    ...
    begin
      ...
      if EOF and CanModify and (not ReadOnly) and (dgEditing in Options) then
        if not (eoDisableInsert in FExOptions) and not ItAddLastRecord then
          AppendClick(Self)
        else
          Key := 0;
    end;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if (ACol = Original) or (not Columns[ACol-1].ReadOnly {$IFDEF SMForDelphi4} and Columns[ACol-1].Visible {$ENDIF}) then
          begin
            if Col <> ACol then
            begin
              ColExit;
...
{    else
  ...
  procedure CheckTab2(GoForward: Boolean);
  var
    ACol, Original: Integer;
  begin
    ACol := Col;
...
; //    Invalidate; {MS1} // avoid repaint! I see no sense in this. Causes flickering and bad gui feedback in large views with a lot of paint events.
  with Datalink.DataSet do
...
//                       PostMessage(Self.Handle, WM_KeyDown, VK_Tab, 0);
      ...
      if ((Key in [VK_LEFT, VK_RIGHT]) and (dgRowSelect in Options)) or
        ...
        if ((Key = VK_TAB) and not (ssAlt in Shift)) then
          {CheckTab2}CheckTab(not (ssShift in Shift));
    end;
  OnKeyDown := nil;
//  try
    inherited KeyDown(Key, Shift);
...
begin
  ...
  if ((ACol = FLastCol) and (ARow = FLastRow)) or
     ...
     {$IFDEF SMForDelphi2010} (FInternalDrawingStyle <> gdsThemed) or {$ENDIF}
...
function TSMDBGrid.SetFilterFocus(AColumn: TColumn): Boolean;
var
  cr: TRect;
begin
  Result := False;
  ...
  if (eoShowFilterBar in ExOptions) and
     (AColumn is TSMDBColumn) then
  begin
    if (dgIndicator in Options) then
      cr := CellRect(AColumn.Index+1, 0)
    else
      cr := CellRect(AColumn.Index, 0);

    with CmbFilter do
    begin
      Column := TSMDBColumn(AColumn);

      Left := cr.Left;
      Top := cr.Bottom - FILTERBAR_HEIGHT;
      Height := FILTERBAR_HEIGHT;

      if (TSMDBColumn(AColumn).FilterList.Count > 0) then
      begin
        try
          Items.BeginUpdate;
          CmbFilter.Items.Assign(TSMDBColumn(AColumn).FilterList);
          Items.Insert(0, '');
          Style := csDropDown;
          ItemIndex := Items.IndexOf(TSMDBColumn(AColumn).FilterValue);
      ...
      begin
        Items.Clear;
        Style := csSimple;
        Text := TSMDBColumn(AColumn).FilterValue;
        Width := cr.Right - cr.Left;
      end;
      Visible := Assigned(AColumn.Field) and ((AColumn.Field.FieldKind = fkData) or (AColumn.Field.FieldKind = fkInternalCalc)) and TSMDBColumn(AColumn).Filterable;
      if Visible then
        SetFocus;
      Result := True;
    end;
...
//CodeGear QC cases shut be #36327 and #34447
  Cell := MouseCoord(X, Y);
  if (Cell.X < 0) and (Cell.Y < 0) then
    if Assigned(InplaceEditor) and not InplaceEditor.Visible then
    begin
      Field:= SelectedField;
      if Assigned(Field) then
        SetEditText(0, 0, Field.Text);
    end;
  ...
  begin
    ...
    begin
      if SetFilterFocus(Columns[Cell.X - ({FixedCols +} IndicatorOffset)]) then
        exit;
(*
      with CmbFilter do
      begin
        ...
        begin
          ...
          begin
           ...
           Visible := Assigned(Column.Field) and ((Column.Field.FieldKind = fkData) or (Column.Field.FieldKind = fkInternalCalc)) and Column.Filterable;
           if Visible then
             SetFocus;
           Exit;
         end;
...
begin
  ...
  // Serguei Tarassov 2015-09-11
  // TDBGrid multi select compatible behavior
  if not ((ssCtrl in Shift) or (ssShift in Shift)) and
    (DataLink.Active and not (eoCheckBoxSelect in ExOptions)) and (not SelectedRows.CurrentRowSelected) and
    (not Assigned(PopupMenu) or (Button <> mbRight)) then
  begin
    SelectedRows.Clear;
  ...
  if (eoCheckBoxSelect in ExOptions) and (Button <> mbRight) and
...
function ValueMatch(const ValueList, Value: TSMGString): Boolean;
var
  Pos: Integer;
begin
  Result := False;
...
begin
  if IsXPThemesEnabled and
     {$IFDEF SMForDelphi2010} (FInternalDrawingStyle = gdsThemed) and {$ENDIF}
     not (csDesigning in ComponentState) then
...
function TSMDBGrid.GetCheckBoxValue(AField: TField; Value: Variant; ValueChecked: TSMGString): TCheckBoxState;
begin
  Result := cbUnChecked;
  try
    if VarIsNull(Value) or VarIsEmpty(Value) then
    ...
    if ({$IFDEF SMForDelphi6}VarToWideStr{$ELSE}VarToStr{$ENDIF}(Value) = ValueChecked) then
      Result := cbChecked
    else
    ...
    begin
      ...
      if (AField is TStringField) and (AField.AsString = 'True') then
        Result := cbChecked
    end
  except
  end;
...
{$IFDEF SMForDelphiXE2}
  ...
  begin
    if StyleServices.GetElementSize(Canvas.Handle, StyleServices.GetElementDetails(tbCheckBoxCheckedNormal),
        DrawRect, esActual, BoxSize{$IFDEF SMForDelphiRX103}, CurrentPPI{$ENDIF}) then
    begin
...
{$IFDEF SMForDelphiXE2}
  ...
  begin
    ...
    StyleServices.DrawElement(Canvas.Handle, Details, DrawRect{R}, nil{$IFDEF SMForDelphiRX103}, CurrentPPI{$ENDIF});
    IsControlDrawn := True;
  end;
{$ENDIF}
...
procedure TSMDBGrid.DrawGraphic(R: TRect; Field: TField);
  ...
  function GetBlobGraphic: TGraphic;
  var
    bs: TMemoryStream;
  begin
    ...
    try
      TBLOBField(Field).SaveToStream(bs);
...
begin
  ...
  try
    try
//      pic.Assign(GetBlobGraphic());
      pic.Assign(TBLOBField(Field));
    except
    end;
...
function TSMDBGrid.GetVerticalAlignment(AColumn: {$IFDEF USE_TNT} TTntColumn {$ELSE} TColumn{$ENDIF}): TSMGVerticalAlignment;
begin
  Result := gvaCenter;
  if Assigned(AColumn) and (AColumn is TSMDBColumn) then
    Result := TSMDBColumn(AColumn).VerticalAlignment
end;
...
var
  ...
  Highlight: Boolean;
  Value: TSMGString;
  DrawColumn: {$IFDEF USE_TNT} TTntColumn {$ELSE} TColumn{$ENDIF};
  FrameOffs: Byte;
...
{$ENDIF}
  ...
  begin
{$IFDEF SMForDelphiXE2}
    if IsCustomStyle {$IFDEF SMForDelphiXE3} and (seClient in StyleElements) {$ENDIF} then
...
//              Canvas.FillRect(ARect);
              WriteTitleText(Canvas, ARect, 2, 2, Value, DrawColumn.Alignment, GetVerticalAlignment(DrawColumn),
                             ...
                             (eoCellWordWrap in ExOptions) //False
                             {$IFDEF SMForDelphi4},
                             (BiDiMode <> bdLeftToRight)
...
{20050201}
          ...
          //! Speed up. Gets called in calling function DrawCell anyway.
...
{  function RowIsMultiSelected: Boolean;
  ...
  procedure DrawTitleColumn(DrawColumn: {$IFDEF USE_TNT} TTntColumn {$ELSE} TColumn{$ENDIF});
  var
    HRect: TRect;
...
{$ELSE}
        ...
        begin
          try
            DrawThemeBackground(hhTheme, Canvas.Handle, HP_HEADERITEM, HIS_NORMAL, @HRect, nil);
          finally
            CloseThemeData(hhTheme);
          end;
        end
...
{
a      Canvas.Brush.Color := DrawColumn.Title.Color;
a      Canvas.FillRect(ARect);
}
      // DrawHighlightCell will set the appropriate font color for highlighted cells
      ApplyStyleColors(Canvas);
...
begin
  {fix for FireDac: very large width in field will create long column}
...
exit;
    ...
    ColDrawn := False;
    {$IFDEF SMForDelphiXE2}
    if StyleServices.Enabled {$IFDEF SMForDelphiXE3} and (seClient in StyleElements) {$ENDIF} then
    begin
      ...
      begin
        if (Assigned(DrawColumn) and (LColor <> clNone) and (DrawColumn.Title.Color = FixedColor)) or
           (not Assigned(DrawColumn) {and (LColor = FixedColor)}) then
          ColDrawn := True;
      end
    end;
    if ColDrawn then
...
//          InflateRect(ARect, -1, -1);
  ...
  begin
    ...
    if Assigned(DrawColumn) then
    begin
      //! Speed up. This must not be called for all cells!?
      //SMDrawGradient(Canvas, ARect, GridStyle.Title.StartColor, GridStyle.Title.EndColor, GridStyle.Title.Direction, 255);
...
//      FOnDrawColumnTitle(Self, ARect, ACol, DrawColumn{Columns[BCol]}, AState);
  ...
  begin
    if (eoTitleLines in ExOptions) then
    begin
      if not (dgColLines in Options) then
      begin
        {$IFDEF SMForDelphi4}
        if (BiDiMode <> bdLeftToRight) then //UseRightToLeftAlignment
        begin
...
//aaa       not ColumnIsCheckbox(DrawColumn) and
    ...
    begin
      ...
      begin
        if GridStyle.IsDefaultBackground and
           not (eoCellWordWrap in ExOptions) and
...
(*  not implemented now
    ...
    begin
      ...
      if DataLink.DataSet.RecNo <> -1 then
        TitleText := IntToStr(DataLink.DataSet.RecNo + ARow - Row)
      else
...
{$ENDIF}
    ...
    if DrawColumn.InheritsFrom(TSMDBColumn) and not TSMDBColumn(DrawColumn).Filterable then
    begin
      SavePen:= Canvas.Pen.Color;
      Canvas.Pen.Color := clWindowFrame;
      Canvas.MoveTo(FixRect.Left, FixRect.Top+1);
      Canvas.LineTo(FixRect.Right, FixRect.Bottom);
      Canvas.MoveTo(FixRect.Left, FixRect.Bottom);
      Canvas.LineTo(FixRect.Right, FixRect.Top+1);
      Canvas.Pen.Color := SavePen;
    end;

    if (DrawColumn is TSMDBColumn) and TSMDBColumn(DrawColumn).Filterable then
    begin
      Canvas.Font := DrawColumn.Title.Font;
{$IFDEF SMForDelphiXE2}
      if TStyleManager.IsCustomStyleActive {$IFDEF SMForDelphiXE3} and (seFont in StyleElements) {$ENDIF} then
        Canvas.Font.Color := StyleServices.GetStyleFontColor(sfGridItemNormal);
{$ENDIF}
...
//          intGroupIndex := i;
          ...
          begin
            ...
            begin
              fld := Datasource.DataSet.FindField(Expression);
              if Assigned(fld) then
                al := fld.Alignment;
            end;
            WriteTitleText(Canvas, RectLookup, -2 + Ord(gdFixed in State), 2, str, al, gvaCenter,
                           teNone,
                           eoTitleWordWrap in ExOptions
                           {$IFDEF SMForDelphi4},
                           (BiDiMode <> bdLeftToRight)
...
function TSMDBGrid.ApplyBackroundColorToCanvas(DataCol, ARow: Integer; AColumn: TColumn; State: TGridDrawState): TColor;
var
...
//    Result := FixedColor
  ...
  begin
    if Highlight or
//       (Assigned(Field) and HighlightCell(DataCol, DataLink.ActiveRecord, Field.AsString, State)) then
    ...
    begin
      // If you have 2+ grids on a form, you cannot see which grid is focused if both have dgAlwaysShowSelection set
      if not Focused then
...
{$IFDEF SMForDelphiXE2}
      ...
      begin
        if {$IFDEF SMForDelphiXE3} (seClient in StyleElements) and {$ENDIF}
           StyleServices.GetElementColor(StyleServices.GetElementDetails(tgClassicCellSelected), ecFillColor, LColor) and
           (LColor <> clNone) then
          Result := LColor;
        if {$IFDEF SMForDelphiXE3} (seFont in StyleElements) and {$ENDIF}
...
{$ENDIF}
    ...
    begin
      ...
      if (AColumn.Color = AColumn.DefaultColor) then
        Result := {AColumn.Color;// }Canvas.Brush.Color
      else
        Result := AColumn.Color;
{$IFDEF SMForDelphiXE2}
      if StyleServices.Enabled and (Result = StyleServices.GetStyleColor(scGrid)) {$IFDEF SMForDelphiXE3} and (seClient in StyleElements) {$ENDIF} then
      begin
...
begin
  ...
  begin
    ...
    if (W > (Rect.Bottom - Rect.Top)) then
  ...
  Canvas.Brush.Color := ApplyBackroundColorToCanvas(DataCol, DataLink.ActiveRecord, Column, State);
  Field := Column.Field;
  Highlight := (gdSelected in State) and
               ((dgAlwaysShowSelection in Options) or Focused);

  if DefaultDrawing and Highlight then
  begin
    if (GridStyle.Selection.Direction <> fdNone) then
    begin
      SMDrawGradient(Canvas, Rect, GridStyle.Selection.StartColor, GridStyle.Selection.EndColor, GridStyle.Selection.Direction, 255);
    end;
  end;
...
//20051027     (GridStyle.Style <> gsNormal) and
  ...
  begin
    ...
    DefaultDrawColumnCell(Rect, DataCol, {$IFDEF USE_TNT} TTntColumn(Column) {$ELSE} Column {$ENDIF}, State);
  end;
  ...
  begin
    if ShowMemoText and
       (Column.Field.DataType in [ftMemo, ftFmtMemo {$IFDEF SMForDelphi2006}, ftWideMemo{$ENDIF}]) then
    begin
      Canvas.FillRect(Rect);
      WriteTitleText(Canvas, Rect, 2, 2, Column.Field.AsString, Column.Alignment, GetVerticalAlignment(Column),
                     GetColumnEllipsis(Column),
                     True
                     {$IFDEF SMForDelphi4},
                     (BiDiMode <> bdLeftToRight)
                     {$ENDIF});
  ...
  begin
    Canvas.FillRect(Rect);
    CheckState := GetCheckBoxValue(Column.Field, Column.Field.Value, '');
...
//      DrawFrameControl(Canvas.Handle, RectLookup, DFC_CAPTION, DFCS_CAPTIONHELP)
      ...
      intMidY := (RectLookup.Bottom - RectLookup.Top) shr 1;
      W := (RectLookup.Bottom - RectLookup.Top) shr 3;
      if W = 0 then W := 1;
      PatBlt(Canvas.Handle, RectLookup.Left + intMidX, RectLookup.Top + intMidY, W, W, BLACKNESS);
      PatBlt(Canvas.Handle, RectLookup.Left + intMidX - (W * 2), RectLookup.Top + intMidY, W, W, BLACKNESS);
      PatBlt(Canvas.Handle, RectLookup.Left + intMidX + (W * 2), RectLookup.Top + intMidY, W, W, BLACKNESS);
    end
    else
    begin
      if DefaultDrawing and
         not ColumnDrawn then
      begin
        if Highlight and
           (GridStyle.Selection.Direction <> fdNone) then
        begin
          if Assigned(Field) then
            Value := Field.DisplayText
          else
            Value := '';
          WriteTitleText(Canvas, Rect, 2, 2, Value, Column.Alignment, GetVerticalAlignment(Column),
                         GetColumnEllipsis(Column),
                         (eoCellWordWrap in ExOptions) //False
                         {$IFDEF SMForDelphi4},
                         (BiDiMode <> bdLeftToRight)
...
{fix for Delphi 10 Seattle:
https://quality.embarcadero.com/browse/RSP-12433}
procedure TSMDBGrid.ChangeScale(M, D: Integer); 
...
procedure TSMDBGrid.FocusFilterBar;
var
  r: TRect;
begin
  if (eoShowFilterBar in ExOptions) then
  begin
    r := CellRect(IndicatorOffset + SelectedIndex, 0);
    if (r.Left <> r.Right) then
      MouseDown(mbLeft, [], r.Left + 5, r.Bottom - FILTERBAR_HEIGHT + 5);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      // Sometimes AV when State=dsEdit and applying filter
      if DataLink.Active then
...
procedure TSMDBGrid.CmbFilterKeyPress(Sender: TObject; var Key: Char);
begin
  if (eoAnyKeyFilter in ExOptions) then
  begin
...
begin
  ...
  begin
    ...
    begin
      col := TSMDBColumn(Columns[i]);
      if col.Filterable then
      begin
        intPos := Pos('*', col.FilterValue);
        IsWildCard := (intPos > 0);
        strFValue := col.FilterValue;
        Delete(strFValue, intPos, 1);
        Accept := (strFValue = '') or
                  ((not IsWildCard) and (col.Field is TNumericField) and (col.Field.DisplayText{AsString} = strFValue)) or
//                  ((not (Field is TNumericField) or IsWildCard) and (Pos(UpperAccentCase(strFValue), UpperAccentCase(Field.AsString)) > 0));
                  ((not (col.Field is TNumericField) or IsWildCard) and
                   ((not Assigned(OnAccentStringConvert) and (Pos(strFValue, col.Field.DisplayText{AsString})>0)) or
                    (Assigned(OnAccentStringConvert) and (Pos(OnAccentStringConvert(Self, strFValue), OnAccentStringConvert(Self, col.Field.DisplayText{AsString})) > 0)))
                  );

        if (col.FilterValue <> '') and Assigned(FOnFilterApply) then
          FOnFilterApply(Self, col.Field, col.FilterValue, Accept);
      end;
      if not Accept then
        break;
    end;
...
function TSMDBGrid.FilterIsEmpty: Boolean;
var
  i: Integer;
begin
  Result := True;
  for i := 0 to Columns.Count - 1 do
    if (Columns[i] is TSMDBColumn) then
      if (TSMDBColumn(Columns[i]).FilterValue <> '') then
      begin
        Result := False;
        break
      end;
...
begin
  ...
  if Assigned(CmbFilter) then
  begin
    CmbFilter.Clear;
    if CmbFilter.Visible then
...
procedure TSMDBGrid.CheckFilter;
begin
  if (eoFilterAutoApply in FExOptions) and not FilterIsEmpty then
    ApplyFilter;
end;
...
initialization
  ...
  RegisterClass(TSMDBGridFilterComboBox);
...
finalization
  ...
  UnRegisterClass(TSMDBGridFilterComboBox);
